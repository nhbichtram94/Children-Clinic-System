    ...
    public class DatKhamService
    {
        ...
        public int TaoPhieuKham(int benhNhanID, int giamHoID, int chuyenKhoaID, DateTime? ngayKham, int khungGioID)
        {
            if (!ngayKham.HasValue)
                throw new ArgumentNullException(nameof(ngayKham), "Ngày khám không được để trống");

            if (benhNhanID <= 0 || giamHoID <= 0 || chuyenKhoaID <= 0 || khungGioID <= 0 || ngayKham.Value.Date < DateTime.Today)
                return 0;
            ...
            using (SqlConnection conn = new SqlConnection(_connString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("TaoPhieuKham", conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@NgayKham", ngayKham.Value);
